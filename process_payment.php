...
<?php
    ...
    if (($_SERVER['REQUEST_METHOD'] == 'POST') || ($_SERVER['REQUEST_METHOD'] == 'GET')){
        ...
        $MaNganHang = isset($_POST['payment']) ? $_POST['payment'] : '';

        $maChuyenDi = isset($_GET['MaChuyenDi']) ? $_GET['MaChuyenDi'] : '';
        switch ($MaNganHang) {
            case 'ZaloPay':
                $MaNganHang = '1';  // Mã ngân hàng giả định cho ZaloPay
                break;
            case 'MoMo':
                $MaNganHang = '2';  // Mã ngân hàng giả định cho MoMo
                break;
            case 'VNPay':
                $MaNganHang = '3';  // Mã ngân hàng giả định cho VNPay
                break;
            case 'VietComBank':
                $MaNganHang = '4';  // Mã ngân hàng giả định cho VietComBank
                break;
            default:
                $MaNganHang = '';  // Giá trị mặc định nếu không có lựa chọn
                break;
        }
        ...
        if ($result->num_rows > 0) {
            ...
            $sql_insert = "INSERT INTO KhachHang (MaKhachHang, HoTen, SoDienThoai, DiaChi, MaNganHang) VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql_insert);
            $stmt->bind_param("ssssi", $MaKhachHang, $name, $SoDienThoai, $DiaChi, $MaNganHang);
            $stmt->execute();
            $MaKhachHang = $stmt->insert_id;
        }

        // Thêm phiếu đặt xe vào cơ sở dữ liệu
        $sql_insert_phieu = "INSERT INTO PhieuDatXe (MaKhachHang, MaChuyenDi, SoGhe, ThoiGianDat, TongSoTien) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql_insert_phieu);
        $stmt->bind_param("ssssd", $MaKhachHang, $maChuyenDi, $seatCount, $ThoiGianDat, $totalAmount);
        if ($stmt->execute()) {
...
<body> 
    ...
    <div class="payment-container">
        <div class="payment-section">
            ...
            <p><strong>Số điện thoại:</strong> <span><?php echo htmlspecialchars($SoDienThoai); ?></span></p>
